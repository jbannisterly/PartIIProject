...
int main(){
    ...
    Mat image(config.barcodeHeight, config.barcodeWidth, CV_8UC3);
    ...
    try {
        Mat recoveredImage = imread("output/img/integration.png");
        ...
        std::vector<uint8_t> recoveredImageData = ImageAux::MatToBytes(image);
        std::vector<uint8_t> recoveredImageDataCompare(recoveredImageData.begin(), recoveredImageData.begin() + imageData.size());
        if (recoveredImageDataCompare != imageData) {
            std::cout << "Image data different" << std::endl;
            exit(-1);
        }
        ...
        if (recoveredPixelsCompare != pixelData) {
            std::cout << "Pixel data different" << std::endl;
            exit(-1);
        }
        ...
        int recoveredLen = HeaderData::GetCompressedLen(recoveredPixels, colourPix, errorCorrectors[0]);
        int nPixels = HeaderData::GetErrorCorrectionLen(recoveredLen, errorCorrectors);

        std::vector<std::vector<uint8_t>> recoveredErrorSplit = colourPix.PixelsToData(recoveredPixels, 0, nPixels);
        if (recoveredErrorSplit != errorSplitData) {
            std::cout << "Error split different" << std::endl;
            exit(-1);
        }

        SplitError splitError(errorCorrectors);
        std::vector<std::vector<uint8_t>> recoveredSplit = splitError.Decode(recoveredErrorSplit);
        if (recoveredSplit != splitData) {
            std::cout << "Split data different" << std::endl;
            exit(-1);
        }

        std::vector recoveredCompressed = SplitBytes::Decode(recoveredSplit, recoveredLen + 2);
        if (recoveredCompressed != compressedData) {
            std::cout << "Compressed data different" << std::endl;
            exit(-1);
        }        

        std::vector<uint8_t> decompressed = Compression::decompress(recoveredCompressed.data() + 2, recoveredLen); 
